...
export const facebook = {
  ...
  async uploadReel({ videoPath, snippet, onProgress }) {
    ...
    //
    // rupload บังคับให้ส่ง Content-Length คู่กับ X-Entity-Length
    // ถ้าส่ง body เป็น stream, Node จะใช้ chunked encoding แล้วไม่มี Content-Length
    // → "Invalid Header format: expected either both Content-Length and
    //    X-Entity-Length, or Transfer-Encoding alone"
    // จึงต้องอ่านไฟล์เป็น Buffer ก่อน (คลิปนิทานราว 10-45MB พอไหว)
    ...
    const up = await fetch(`${RUPLOAD}/${s.video_id}`, {
      ...
      headers: {
        ...
        'X-Entity-Name': `reel_${s.video_id}.mp4`,
  ...
  /**
   * โพสต์ภาพปกพร้อมแคปชัน (ไม่ใช่วิดีโอ)
   *
   * ใช้เมื่ออยากโพสต์เป็นรูปภาพ ซึ่งได้ reach คนละทางกับวิดีโอ
   * แคปชันเขียนล่วงหน้าเก็บใน story_captions เพื่อไม่ให้ทุกโพสต์
   * หน้าตาเหมือนกัน (Facebook ลด reach ถ้าเนื้อหาซ้ำแบบ)
   *
   * @param {{photoPath:string, caption:string, links?:{label:string,url:string}[]}}
   */
  async postPhoto({ photoPath, caption, links = [], onProgress }) {
    const page = process.env.FB_PAGE_ID;
    const token = process.env.FB_PAGE_TOKEN;
    onProgress?.(10);

    let text = caption || '';
    // ลิงก์อ้างอิง เช่น ชี้จากโพสต์ภาพ/คลิปสั้น ไปคลิปเต็ม
    if (links.length) {
      text += '\n\n' + links.map((l) => `${l.label}\n${l.url}`).join('\n\n');
    }
    if (!text.includes(AI_NOTE)) text += `\n\n${AI_NOTE}`;
    ...
    const r = await fetch(`${GRAPH}/${page}/photos`, { method: 'POST', body: form });
    const d = await r.json();
    if (!r.ok || !d.id) {
      throw new Error(`โพสต์ภาพไม่สำเร็จ: ${JSON.stringify(d).slice(0, 300)}`);
    }
    onProgress?.(100);
    return {
      remote_id: d.post_id || d.id,
      remote_url: `https://www.facebook.com/${d.post_id || d.id}`,
    };
  },

  /**
   * เพิ่มคอมเมนต์ใต้โพสต์ — ใช้แปะลิงก์คลิปเต็มใต้โพสต์คลิปสั้น
   *
   * ทำไมใส่ในคอมเมนต์ไม่ใช่ในแคปชัน:
   * Facebook ลด reach โพสต์ที่มีลิงก์ออกนอกแพลตฟอร์มในตัวโพสต์
   * แต่ลิงก์ในคอมเมนต์ไม่โดนลด
   */
  async comment(postId, message) {
    const token = process.env.FB_PAGE_TOKEN;
    const r = await fetch(`${GRAPH}/${postId}/comments`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message, access_token: token }),
    });
    const d = await r.json();
    if (!r.ok || !d.id) throw new Error(JSON.stringify(d).slice(0, 200));
    return d.id;
  },
