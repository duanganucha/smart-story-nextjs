...
// Downscaled WebP derivatives written next to every scene PNG. The full-res PNG
// stays the archive copy; clients pick a variant by how big they draw it:
//   _thumb (~20 KB) → library grid cards, filmstrip strip
//   _med   (~100 KB) → the player's main image
// A 1376x768 PNG is ~1.9 MB, so the grid drops from megabytes to kilobytes.
export const IMAGE_VARIANTS = [
  { suffix: '_thumb', width: 400, quality: 70 },
  { suffix: '_med', width: 1024, quality: 80 },
];

/** `/scenes/7/scene_01.png` → `/scenes/7/scene_01_thumb.webp` */
export function variantPath(pngPath, suffix) {
  if (!pngPath) return null;
  return pngPath.replace(/\.png(\?|$)/i, `${suffix}.webp$1`);
}

// Build the WebP derivatives for one freshly written PNG. Never throws: a
// missing thumbnail must not fail a scene that rendered fine, because clients
// fall back to the PNG.
export async function writeImageVariants(pngFile) {
  await Promise.all(
    IMAGE_VARIANTS.map(async ({ suffix, width, quality }) => {
      const out = pngFile.replace(/\.png$/i, `${suffix}.webp`);
      try {
        ...
        console.error(`[scenes] variant ${suffix} failed for ${pngFile}:`, e?.message || e);
      }
    })
  );
}
...
async function renderImage(engine, fullPrompt, file, aspectRatio, attempts = IMG_ATTEMPTS) {
  ...
  for (let a = 0; a < attempts; a++) {
    try {
      ...
      // Every scene image — new story or single-scene regen — flows through
      // here, so this is the one place that keeps derivatives in sync.
      await writeImageVariants(file);
      return; // success
    } catch (e) {
      lastErr = e;
